...
int feature_extraction_pipeline(int argc, char** argv)
{
	...
	if (argc < num_required_args)
	{
		gflags::SetUsageMessage(
			...
			"Usage: feature_extraction.exe  pretrained_net_param"
	...
	char fe_file[1024];
	sprintf(fe_file, "%s.dat", argv[4]);

	FILE *fp = NULL;
	fopen_s(&fp, fe_file, "wb+");
		...
		for (int i = 0; i < num_features; ++i)
		{
			...
			for (int n = 0; n < batch_size; ++n)
			{
				...
				for (int d = 0; d < dim_features; ++d)
				{
					...
					fFeature[d] = feature_blob_data[d];
				}
				fwrite(fFeature, sizeof(fFeature), 1, fp);
