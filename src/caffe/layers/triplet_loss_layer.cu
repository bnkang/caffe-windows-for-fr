...
namespace caffe {
	...
	void TripletLossLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top)
	{
		...
		if (losstype == 0)
		{
			for (int i = 0; i < num_set; ++i)
			{
				...
				for (int triplet = 0; triplet < num_negatives; ++triplet)
				{
					...
					//printf("[#%d] = %f, ", triplet, dist_sq_neg.cpu_data()[i]);
		...
		else if (losstype == 3)
		{
			for (int i = 0; i < num_set; ++i)
			{
				caffe_gpu_sub(
					...
					bottom[0]->gpu_data() + (2 + num_negatives)*i*dim, // reference
					bottom[0]->gpu_data() + ((2 + num_negatives)*i + 1)*dim, // positive
					diff_pos.mutable_gpu_data() + i*dim); // reference - positive
				...
				//caffe_gpu_powx(1, dist_sq_pos.cpu_data() + i, Dtype(0.5), dist_pos.mutable_cpu_data() + i);
				dist_pos.mutable_cpu_data()[i] = pow(dist_sq_pos.cpu_data()[i], 0.5);

				// a b is a similar pair for pair wise
				// loss accumulated by the piar wise part
				if (use_pair == 1)
				{
					loss += dist_sq_pos.cpu_data()[i];
				}
				//printf("A-P: %f, ", dist_pos.cpu_data()[i]);
				//printf(" A-N: ");
				for (int triplet = 0; triplet < num_negatives; ++triplet)
				{
					//dist_sq_.mutable_cpu_data()[i] = dist_sq_pos.mutable_cpu_data()[i];
					dist_sq_.mutable_cpu_data()[i] = dist_pos.mutable_cpu_data()[i];
					dist_sq_.mutable_cpu_data()[i] += margin;
					...
					caffe_gpu_sub(
						dim,
						bottom[0]->gpu_data() + (2 + num_negatives)*i*dim, // reference
						bottom[0]->gpu_data() + ((2 + num_negatives)*i + 2 + triplet) * dim, // negative
						diff_neg.mutable_gpu_data() + i *dim); // reference - negative
					...
					//caffe_gpu_powx(1, dist_sq_neg.cpu_data() + i, Dtype(0.5), dist_neg.mutable_cpu_data() + i);
					dist_neg.mutable_cpu_data()[i] = pow(dist_sq_neg.cpu_data()[i], 0.5);
					//printf("[#%d] = %f, ", triplet, dist_neg.cpu_data()[i]);

					//LOG(INFO) << "#" << i << "A-P: " << dist_pos.cpu_data()[i] << ", A-N: " << dist_neg.cpu_data()[i];

					// a and nagative[triplet] is a dissimilar pair for triplet
					dist_sq_.mutable_cpu_data()[i] = 1 - \
						dist_neg.cpu_data()[i] / dist_sq_.mutable_cpu_data()[i];

					// loss accumulated by the triplet part
					loss += std::max(dist_sq_.cpu_data()[i], Dtype(0.0));
				}
				//printf("\n");
			}
		...
		else if (losstype == 4)
		{
			for (int i = 0; i < num_set; ++i)
			{
				caffe_gpu_sub(
					...
					bottom[0]->gpu_data() + (2 + num_negatives)*i*dim, // reference
					bottom[0]->gpu_data() + ((2 + num_negatives)*i + 1)*dim, // positive
					diff_pos.mutable_gpu_data() + i*dim); // r - p
				caffe_gpu_dot(
					dim,
					diff_pos.gpu_data() + i*dim,
					diff_pos.gpu_data() + i*dim,
				...
				if (use_pair == 1)
				{
					loss += 0.5 * dist_sq_pos.cpu_data()[i];
				}
				...
				for (int triplet = 0; triplet < num_negatives; ++triplet)
				{
					...
					caffe_gpu_sub(
						...
						bottom[0]->gpu_data() + (2 + num_negatives)*i*dim, // reference
						bottom[0]->gpu_data() + ((2 + num_negatives)*i + 2 + triplet)*dim, // negative
						diff_neg.mutable_gpu_data() + i*dim); // r - n
					caffe_gpu_dot(
						dim,
						diff_neg.gpu_data() + i*dim,
						diff_neg.gpu_data() + i*dim,
					...
					dist_sq_.mutable_cpu_data()[i] -= dist_neg.cpu_data()[i];
		...
		else if (losstype == 5)
		{
			for (int i = 0; i < num_set; ++i)
			{
				...
				caffe_gpu_sub(
					dim,
					bottom[0]->gpu_data() + ((2 + num_negatives)*i + 2 + 1)*dim, // negative_1
					bottom[0]->gpu_data() + ((2 + num_negatives)*i + 2 + 2)*dim, // negative_2
					diff_neg.mutable_gpu_data() + i*dim); // n1 - n2
				...
				LOG(INFO) << "#" << i << "A-P: " << dist_pos.cpu_data()[i] << ", N1-N2: " << dist_neg.cpu_data()[i];
	...
	template <typename Dtype>
	void TripletLossLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top, const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom) 
	{
		Dtype margin = this->layer_param_.triplet_loss_param().margin();
		Dtype losstype = this->layer_param_.triplet_loss_param().losstype();
			...
			for (int i = 2; i < 2 + num_negatives; ++i) 
			{
				if (propagate_down[0]) 
				{
					...
					for (int j = 0; j < num_set; ++j) 
					{
						...
						{
							caffe_gpu_set(dim, Dtype(0), bout + ((2 + num_negatives)*j + i)*dim);
						}
					}
				}
			}
			...
			for (int i = 0; i < 1; ++i) 
			{
				...
				if (propagate_down[0]) 
				{
					...
					for (int j = 0; j < num_set; ++j) 
					{
						...
						for (int triplet = 0; triplet < num_negatives; ++triplet) 
						{
							dist_sq_.mutable_cpu_data()[j] = exp(dist_sq_pos.mutable_cpu_data()[j]);
							dist_sq_.mutable_cpu_data()[j] += margin;
			...
			for (int i = 1; i < 2; ++i) 
			{
				...
				if (propagate_down[0]) 
				{
					...
					for (int j = 0; j < num_set; ++j) 
					{
						...
						for (int triplet = 0; triplet < num_negatives; ++triplet)
						{
							dist_sq_.mutable_cpu_data()[j] = exp(dist_sq_pos.cpu_data()[j]);
							dist_sq_.mutable_cpu_data()[j] += margin;
				...
				if (propagate_down[0])
				{
					...
					for (int j = 0; j < num_set; ++j)
					{
						...
						caffe_gpu_dot(
							...
							diff_pos.gpu_data() + (j*dim),
							diff_pos.gpu_data() + (j*dim),
							dist_sq_pos.mutable_cpu_data() + j);

						//caffe_gpu_powx(1, dist_sq_pos.cpu_data() + j, Dtype(0.5), dist_pos.mutable_cpu_data() + j);
						dist_pos.mutable_cpu_data()[j] = pow(dist_sq_pos.cpu_data()[j], 0.5);

						// the num_negatives triplet part
						for (int triplet = 0; triplet < num_negatives; ++triplet)
						{
							//dist_sq_.mutable_cpu_data()[j] = dist_sq_pos.cpu_data()[j];
							dist_sq_.mutable_cpu_data()[j] = dist_pos.cpu_data()[j];
							dist_sq_.mutable_cpu_data()[j] += margin;
							...
							//caffe_gpu_powx(1, dist_sq_neg.cpu_data() + j, Dtype(0.5), dist_neg.mutable_cpu_data() + j);
							dist_neg.mutable_cpu_data()[j] = pow(dist_sq_neg.cpu_data()[j], 0.5);

							// a and negative[triplet] is a dissimilar pair for triplet
							dist_sq_.mutable_cpu_data()[j] = 1 - \
								dist_neg.cpu_data()[j] / dist_sq_.cpu_data()[j];
							...
							if ((dist_sq_.cpu_data()[j]) > Dtype(0.0))
							{
								caffe_gpu_axpby(
									dim,
									alpha*dist_neg.mutable_cpu_data()[j] / (dist_pos.cpu_data()[j] *
									(dist_pos.cpu_data()[j] + margin) * (dist_pos.cpu_data()[j] + margin)),
									diff_pos.gpu_data() + (j*dim),
									Dtype(1.0),
									bout + ((2 + num_negatives)*j + i)*dim);
								...
								caffe_gpu_axpby(
									dim,
									-alpha / (dist_neg.cpu_data()[j] * ((dist_pos.cpu_data()[j] + margin) *
									(dist_pos.cpu_data()[j] + margin))),
									diff_neg.gpu_data() + (j*dim),
									Dtype(1.0),
									bout + ((2 + num_negatives)*j + i)*dim);
							}
				...
				if (propagate_down[0])
				{
					...
					for (int j = 0; j < num_set; ++j)
					{
						...
						for (int triplet = 0; triplet < num_negatives; ++triplet)
						{
							...
							// a and negative[triplet] is a dissimilar pair for triplet
							dist_sq_.mutable_cpu_data()[j] = 1 - \
								dist_neg.cpu_data()[j] / dist_sq_.mutable_cpu_data()[j];
							...
							if ((dist_sq_.cpu_data()[j]) > Dtype(0.0))
							{
								caffe_gpu_axpby(
									dim,
									alpha*dist_neg.cpu_data()[j] / (dist_pos.cpu_data()[j] *
									((dist_pos.cpu_data()[j] + margin) * (dist_pos.cpu_data()[j] + margin))),
									diff_pos.gpu_data() + (j*dim),
									Dtype(1.0),
									bout + ((2 + num_negatives)*j + i) * dim);
				...
				if (propagate_down[0])
				{
					...
					for (int j = 0; j < num_set; ++j)
					{
						...
						for (int triplet = 0; triplet < num_negatives; ++triplet)
						{
							...
							dist_sq_.mutable_cpu_data()[j] = dist_pos.cpu_data()[j];
							dist_sq_.mutable_cpu_data()[j] -= dist_neg.cpu_data()[j];

							if (margin + dist_sq_.cpu_data()[j] > Dtype(0.0))
							{
								caffe_gpu_axpby(
									dim,
									alpha / dist_pos.cpu_data()[j],
									diff_pos.cpu_data() + (j*dim),
									Dtype(1.0),
									bout + ((2 + num_negatives)*j+i)*dim);
								caffe_gpu_axpby(
									dim,
									-alpha / dist_neg.cpu_data()[j],
									diff_neg.gpu_data() + (j*dim),
									Dtype(1.0),
									bout + ((2 + num_negatives)*j+i)*dim);
							}							
