...
#include "caffe/layers/subregion_layer.hpp"

namespace caffe {

	template <typename Dtype>
	void SubRegionLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top)
	{
	...
	template <typename Dtype>
	void SubRegionLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) 
	{
	...
	template <typename Dtype>
	void SubRegionLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) 
	{
		...
		for (int n = 0; n < num; n++) 
		{
			for (int i = 0; i < num_point; i++) 
			{
				...
				if (top.size() == 3) 
				{
					...
					top[1]->mutable_cpu_data()[n * bottom[1]->shape(1) + i * 2] = (ground_truth_point_data[n * bottom[1]->shape(1) + i * 2] + data_width_ / 2) - top[2]->cpu_data()[n * bottom[1]->shape(1) + i * 2];
					top[1]->mutable_cpu_data()[n * bottom[1]->shape(1) + i * 2 + 1] = (ground_truth_point_data[n * bottom[1]->shape(1) + i * 2 + 1] + data_height_ / 2) - top[2]->cpu_data()[n * bottom[1]->shape(1) + i * 2 + 1];
				...
				for (int c = 0; c < channels; c++) 
				{
					for (int h = 0; h < height_; h++) 
					{
						...
						{
							if (y0 + h >= 0 && y0 + h <= bottom[0]->height() - 1 && x0 + w >= 0 && x0 + w <= bottom[0]->width() - 1) 
							{
								if (as_dim_ == 0) 
								{
	...
	template <typename Dtype>
	void SubRegionLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top, const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom) 
	{
		if (!propagate_down[0]) 
		{
			return;
