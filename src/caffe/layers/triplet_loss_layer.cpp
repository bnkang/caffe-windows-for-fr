...
namespace caffe
{
	...
	void TripletLossLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) 
	{
		...
		dist_pos.Reshape(num_set, 1, 1, 1);
		dist_neg.Reshape(num_set, 1, 1, 1);
		
		// for positive - negative pair
		diff_pos_neg.Reshape(num_set, dim, 1, 1);
		dist_sq_pos_neg.Reshape(num_set, 1, 1, 1);
	...
	void TripletLossLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) 
	{
		...
		if (losstype == 0) 
		{
			for (int i = 0; i < num_set; ++i) 
			{
				...
				for (int triplet = 0; triplet < num_negatives; ++triplet) 
				{
					...
					caffe_sub(
						dim,
						bottom[0]->cpu_data() + ((2 + num_negatives)*i + 1)*dim, // positive
						bottom[0]->cpu_data() * ((2 + num_negatives)*i + 2 + triplet) *dim, // negative
						diff_pos_neg.mutable_cpu_data() + i * dim); // positive - negative

					dist_sq_pos_neg.mutable_cpu_data()[i] =
						caffe_cpu_dot(dim,
						diff_pos_neg.cpu_data() + i * dim,
						diff_pos_neg.cpu_data() + i * dim);

					if (use_pair == 1)
					{
						loss -= dist_sq_pos_neg.cpu_data()[i];
					}
					*/
		...
		else if (losstype == 1) 
		{
			for (int i = 0; i < num_set; ++i) 
			{
				caffe_sub(
					dim,
					bottom[0]->cpu_data() + (2 + num_negatives)*i*dim,  // reference
					bottom[0]->cpu_data() + ((2 + num_negatives)*i + 1)*dim,  // positive
					diff_pos.mutable_cpu_data() + i*dim);  // reference-pose_close
		...
		else if (losstype == 3)
		{
			for (int i = 0; i < num_set; ++i)
			{
				caffe_sub(
					...
					bottom[0]->cpu_data() + (2 + num_negatives)*i*dim, // reference
					bottom[0]->cpu_data() + ((2 + num_negatives)*i + 1)*dim, // positive
					diff_pos.mutable_cpu_data() + i *dim); // reference - positive
					...
					for (int j = 0; j < num_set; ++j) 
					{
						...
						for (int triplet = 0; triplet < num_negatives; ++triplet)
						{
							caffe_sub(
								...
								bottom[0]->cpu_data() + ((2 + num_negatives)*j + 2 + triplet)*dim, // negative
					...
					for (int j = 0; j < num_set; ++j) 
					{
						...
						for (int triplet = 0; triplet < num_negatives; ++triplet) 
						{
							...
							if ((dist_sq_.cpu_data()[j]) > Dtype(0.0)) 
							{
								caffe_cpu_axpby(
									dim,
									alpha*dist_sq_neg.cpu_data()[j] / ((dist_sq_pos.cpu_data()[j] + margin)* (dist_sq_pos.cpu_data()[j] + margin)),
									diff_pos.cpu_data() + (j*dim),
									Dtype(1.0),
									bout + ((2 + num_negatives)*j + i)*dim);
			...
			for (int i = 2; i < 2 + num_negatives; ++i)
			{
				...
				if (propagate_down[0])
				{
					...
					for (int j = 0; j < num_set; ++j)
					{
						...
						caffe_sub(
							dim,
							bottom[0]->cpu_data() + ((2 + num_negatives) * j  + 1)*dim, // positive
							bottom[0]->cpu_data() + ((2 + num_negatives) * j + i)*dim, // negative
							diff_pos_neg.mutable_cpu_data() + j * dim); // positive - negative
