...
namespace caffe {
		...
		const Dtype* bottom_data, const Dtype*  point_data, Dtype* top_data) 
	{
		CUDA_KERNEL_LOOP(index, num * channels * target_height * target_width * num_point) 
		{
			...
			int x0 = floor(center_x - target_width / 2);
			int y0 = floor(center_y - target_height / 2);
			if (y0 + h >= 0 && y0 + h <= bottom_height - 1 && x0 + w >= 0 && x0 + w <= bottom_width - 1) 
			{
				if (as_dim == 0) 
				{
	...
	template <typename Dtype>
	void SubRegionLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) 
	{
	...
	template <typename Dtype>
	void SubRegionLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top, const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom) 
	{
		if (!propagate_down[0]) 
		{
			return;
