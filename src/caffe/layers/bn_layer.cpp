...
namespace caffe {
	...
	void BNLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top)
	{
		...
		// Check if we need to set up the wieghts
		if (this->blobs_.size() > 0)
		{
			LOG(INFO) << "Skipping parameter initializattion";
		}
		else
		{
	...
	template <typename Dtype>
	void BNLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) 
	{
		...
		if (this->phase_ == TRAIN) 
		{
			caffe_cpu_axpby(batch_statistic_.count(), Dtype(1) - decay_, batch_statistic_.cpu_data(), decay_, this->blobs_[3]->mutable_cpu_data());
			// add eps
		...
		// std
		caffe_powx(batch_statistic_.count(), batch_statistic_.cpu_data(), Dtype(0.5), batch_statistic_.mutable_cpu_data());
