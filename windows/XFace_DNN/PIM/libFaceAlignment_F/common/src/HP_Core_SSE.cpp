...
{
	__declspec(align(16)) unsigned char mask1[] = {0, 0, 0, 0, 1, 1, 1, 2, 2, 3, 0, 0, 0, 0, 0, 0};
	__declspec(align(16)) unsigned char mask2[] = {1, 2, 3, 4, 2, 3, 4, 3, 4, 4, 0, 0, 0, 0, 0, 0};
	...
	for(int f = 0; f < R->fnum; ++f)
	{
		...
		__m128i temp;
		temp.m128i_u8[0] = interpol_i[0];
		temp.m128i_u8[1] = interpol_i[1];
		temp.m128i_u8[2] = interpol_i[2];
		temp.m128i_u8[3] = interpol_i[3];
		temp.m128i_u8[4] = interpol_i[4];
		...
		int lcode = _mm_movemask_epi8(temp) & 0x3ff;
...
//will to be optimized using SSE 
